...
class VoxelGeneratorWrapper():
    def __init__(self, vsize_xyz, coors_range_xyz, num_point_features, max_num_points_per_voxel, max_num_voxels):
        self.vsize_xyz = np.array(vsize_xyz, dtype=np.float32)
        self.coors_range_xyz = np.array(coors_range_xyz, dtype=np.float32)
        self.max_num_points_per_voxel = max_num_points_per_voxel
        self.max_num_voxels = max_num_voxels
        
        grid_size = (self.coors_range_xyz[3:6] - self.coors_range_xyz[0:3]) / self.vsize_xyz
        ...
        try:
            ...
            self.spconv_ver = 1
            if hasattr(VoxelGenerator, '__name__') and ('Mock' in VoxelGenerator.__name__):
        ...
        # Always fallback to numpy if we detect we are on Jetson without proper spconv
        if self.use_numpy_fallback:
            print("[OpenPCDet] Using native Numpy Voxelization instead of spconv")
        else:
            if self.spconv_ver == 1:
                self._voxel_generator = VoxelGenerator(
                    voxel_size=vsize_xyz, # type: ignore
                    point_cloud_range=coors_range_xyz, # type: ignore
                    max_num_points=max_num_points_per_voxel, # type: ignore
                    max_voxels=max_num_voxels # type: ignore
                )
            else:
                self._voxel_generator = VoxelGenerator(
                    vsize_xyz=vsize_xyz,
    ...
    def generate(self, points):
        if self.use_numpy_fallback:
            import numpy as np
            # Numpy voxelization
            mask = np.all(points[:, :3] >= self.coors_range_xyz[:3], axis=1) & \
                   np.all(points[:, :3] <= self.coors_range_xyz[3:], axis=1)
            points = points[mask]
            
            coords = np.floor((points[:, :3] - self.coors_range_xyz[:3]) / self.vsize_xyz).astype(np.int64)
            
            coords_1d = coords[:, 0] * self.grid_size[1] * self.grid_size[2] + coords[:, 1] * self.grid_size[2] + coords[:, 2]
            _, inverse_indices = np.unique(coords_1d, return_inverse=True)
            
            voxels = np.zeros((len(np.unique(coords_1d)), self.max_num_points_per_voxel, points.shape[1]), dtype=np.float32)
            voxel_coords = np.zeros((len(np.unique(coords_1d)), 3), dtype=np.int32)
            num_points_per_voxel = np.zeros((len(np.unique(coords_1d)),), dtype=np.int32)
            
            for i, idx in enumerate(inverse_indices):
                if num_points_per_voxel[idx] < self.max_num_points_per_voxel:
                    voxels[idx, num_points_per_voxel[idx]] = points[i]
                    num_points_per_voxel[idx] += 1
                    if num_points_per_voxel[idx] == 1:
                        # Z, Y, X order for PointPillars
                        voxel_coords[idx] = coords[i, ::-1]
                        
            if len(voxel_coords) > self.max_num_voxels:
                voxels = voxels[:self.max_num_voxels]
                voxel_coords = voxel_coords[:self.max_num_voxels]
                num_points_per_voxel = num_points_per_voxel[:self.max_num_voxels]
